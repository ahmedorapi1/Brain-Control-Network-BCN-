...
model.load_state_dict(
    torch.load(r'D:\GP_data\EEG\our paper\LSTM+finetuning\best_tuned_model_LSTM_52.78.pth', map_location='cpu',  weights_only=False),
    strict=False
)
...
with torch.no_grad():
    for i, (X_batch, y_batch) in enumerate(loader):
        ...
        batch_indices = list(range(i * batch_size, i * batch_size + batch_size))
...
correct_samples = all_indices[correct_mask]
wrong_samples = all_indices[wrong_mask]
...
np.save("correct_samples.npy", correct_samples)
np.save("wrong_samples.npy", wrong_samples)

print("Saved: inference_results.csv, correct_samples.npy, wrong_samples.npy")
