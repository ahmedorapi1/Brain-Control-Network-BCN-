...
class_counter = {c: 0 for c in range(num_classes)}

# نحول loader iterator
for X_batch, y_batch in loader:

    for i in range(len(X_batch)):
        x = X_batch[i:i+1].numpy().astype(np.float32)
        ...
        if pred == y_true:
            ...
            if class_counter[y_true] < num_per_class:
                selected_X.append(X_batch[i].numpy())
                selected_y.append(y_true)
                class_counter[y_true] += 1

        # لو خلصنا 5 لكل class نوقف
...
selected_y = np.array(selected_y, dtype=np.int64)

print("Final shape X:", selected_X.shape)
print("Final shape y:", selected_y.shape)

np.savez(
    "correct_5per_class.npz",
    X=selected_X,
    y=selected_y
)

print("Saved as .npz successfully ✅")